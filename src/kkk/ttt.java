...
public class ttt extends JFrame {
    ...
    static final Color ACCENT       = new Color(0xF5, 0xB6, 0x00);
    static final Color ACCENT_HOVER = new Color(0xFF, 0xC9, 0x2E);
    static final Color SUCCESS      = new Color(0x1F, 0x8A, 0x5A);
    static final Font  FONT_H1      = new Font("SansSerif", Font.BOLD, 22);
    ...
    static final int MENU_IMG_W = 72,  MENU_IMG_H = 72;
    ...
    private JPanel buildMenuScreen() {
        ...
        JLabel logo = new JLabel("♪ カラオケ予約");
        logo.setFont(new Font("SansSerif", Font.BOLD, 22));
        logo.setForeground(ACCENT);
        logo.setAlignmentX(Component.LEFT_ALIGNMENT);

        JLabel sub = new JLabel("ご希望の操作を選択してください");
        sub.setFont(FONT_BODY);
        sub.setForeground(TEXT_MUTED);
        sub.setAlignmentX(Component.LEFT_ALIGNMENT);
        sub.setBorder(new EmptyBorder(4, 0, 24, 0));

        JButton bookBtn = accentButton("予約する");
        bookBtn.setAlignmentX(Component.LEFT_ALIGNMENT);
        bookBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 44));
        bookBtn.addActionListener(e -> goTo("DASHBOARD"));

        JButton lookupBtn = ghostButton("予約のキャンセル・変更");
        lookupBtn.setAlignmentX(Component.LEFT_ALIGNMENT);
        lookupBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 44));
        ...
        historyBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 44));
    ...
    private JPanel buildDashboardScreen() {
        ...
        JLabel title = new JLabel("空きルーム一覧");
        title.setFont(FONT_H1);
        title.setForeground(TEXT_DARK);
        JLabel sub = new JLabel("ご希望のルームを選択してください");
        sub.setFont(FONT_BODY);
        sub.setForeground(TEXT_MUTED);
        ...
        JButton orderBtn = ghostButton("🍽 フード・ドリンク注文");
    ...
    private JPanel buildRoomCard(Room room) {
        ...
        card.setMaximumSize(new Dimension(Integer.MAX_VALUE, ROOM_IMG_H + 30));
        ...
        capLbl.setBorder(new EmptyBorder(4, 0, 0, 0));

        JLabel hintLbl = new JLabel("タップして予約 →");
        hintLbl.setFont(FONT_LABEL);
        hintLbl.setForeground(ACCENT);
        hintLbl.setAlignmentX(Component.LEFT_ALIGNMENT);
        hintLbl.setBorder(new EmptyBorder(4, 0, 0, 0));

        info.add(nameLbl);
        info.add(capLbl);
        info.add(hintLbl);

        // カード全体・子要素すべてにクリックとホバーの挙動を持たせる
        MouseAdapter cardClick = new MouseAdapter() {
            public void mouseClicked(MouseEvent e) { startNewBooking(room); }
            public void mouseEntered(MouseEvent e) { card.setBackground(new Color(0xEF, 0xEF, 0xF3)); setChildrenBackground(card, new Color(0xEF, 0xEF, 0xF3)); }
    ...
    private JPanel buildBookingScreen() {
        ...
        timeCombo.setFont(new Font("SansSerif", Font.PLAIN, 14));
        timeCombo.setBackground(BG_WHITE);
    ...
    private JPanel buildMyBookingsScreen() {
        ...
        table.setSelectionBackground(new Color(0xFC, 0xE4, 0xDB));
    ...
    private JPanel buildOrderScreen() {
        ...
        JLabel title = new JLabel("フード・ドリンク注文");
        title.setFont(FONT_H1);
        title.setForeground(TEXT_DARK);
        title.setAlignmentX(Component.LEFT_ALIGNMENT);
        title.setBorder(new EmptyBorder(10, 0, 0, 0));

        JLabel sub = new JLabel("ご希望の商品と数量を選択してください");
        sub.setFont(FONT_BODY);
        sub.setForeground(TEXT_MUTED);
        sub.setAlignmentX(Component.LEFT_ALIGNMENT);
        sub.setBorder(new EmptyBorder(4, 0, 16, 0));
        ...
        itemsList.setBackground(BG_WHITE);

        orderSpinners.clear();
        for (MenuItem item : db.getMenuItems()) {
            itemsList.add(buildMenuItemRow(item));
    ...
    // ★変更点: メニュー行に商品画像（左側）を追加
    private JPanel buildMenuItemRow(MenuItem item) {
        JPanel row = new JPanel(new BorderLayout(14, 0));
        row.setBackground(PANEL_GRAY);
        row.setBorder(new CompoundBorder(new LineBorder(BORDER_GRAY, 1, true), new EmptyBorder(10, 14, 10, 14)));
        row.setMaximumSize(new Dimension(Integer.MAX_VALUE, MENU_IMG_H + 22));
        ...
        qtyWrap.setBackground(PANEL_GRAY);
        qtyWrap.add(qtySpinner);

        row.add(thumb, BorderLayout.WEST);
        row.add(info, BorderLayout.CENTER);
        row.add(qtyWrap, BorderLayout.EAST);
        return row;
    }
    ...
    private JTextField styledTextField(String placeholder) {
        ...
        field.addFocusListener(new FocusAdapter() {
            public void focusGained(FocusEvent e) {
                ...
                field.setBorder(new CompoundBorder(new LineBorder(ACCENT, 1, true), new EmptyBorder(8, 12, 8, 12)));
            }
    ...
    private void styleSpinner(JSpinner spinner) {
        ...
        spinner.getEditor().setBackground(BG_WHITE);
    ...
    private JButton ghostButton(String text) {
        ...
        btn.setFocusPainted(false);
        btn.setBorder(new CompoundBorder(new LineBorder(BORDER_GRAY, 1, true), new EmptyBorder(9, 16, 9, 16)));
        btn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btn.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) { btn.setBackground(PANEL_GRAY); }
            public void mouseExited(MouseEvent e)  { btn.setBackground(BG_WHITE); }
        });
        return btn;
    }
    ...
    static class MenuItem {
        ...
        MenuItem(int id, String name, int price, String imagePath) {
            this.id = id;
            this.name = name;
            this.price = price;
            this.imagePath = imagePath;
